# Infrastructure — Session Memory
> Domain-level session log for infrastructure/.
...
This memory file tracks work in: `infrastructure/`
...
- Keycloak (port 8180)
- Caddy
- WunderGraph Cosmo (port 4000)
- PostgreSQL (port 5432)

**Key files:**
- docker-compose.yml (root)
- infrastructure/Caddyfile
- infrastructure/init/postgres/
...
**What was done**:
- infrastructure/AGENTS.md updated — Conductor skill section added
- Architecture snapshot updated — Agno/n8n removed, Conductor OSS added

**Files modified**:
- infrastructure/AGENTS.md — Conductor skill wired in

**Decisions made**:
- Conductor OSS replaces Agno + n8n per ADR-033
- No containers started — CONDUCTOR GATE still active

**HITL gates hit**:
- CONDUCTOR GATE — not lifted (pending first end-to-end ingest test)

**Open threads**:
- No infrastructure tasks approved — gated on CONDUCTOR GATE lift
...
```json
{
  "trace_id": "2026-04-21-infrastructure-1",
  "task": "session continuity — architecture update + tooling",
  "files_modified": ['infrastructure/AGENTS.md'],
