...
export class ImessagePdfParser {
  /**
   * Parse PDF iMessage export
   * Uses pure Node.js pdf-parse for text extraction to avoid brittle python calls
   */
  async parse(filePath: string): Promise<ImessageMessage[]> {
    try {
      // Dynamic import to bypass ESM/CJS compilation interop issues with pdf-parse
      // @ts-ignore
  ...
  private parseExtractedText(text: string): ImessageMessage[] {
    ...
    for (const line of lines) {
      ...
      if (match1) {
        ...
        const [, sender, timestampStr, textContent] = match1;
        currentMessage = {
          sender: sender.trim(),
      ...
      // Pattern 2: "Timestamp - Sender: Message"
        ...
        if (match2) {
          ...
          const [, timestampStr, sender, textContent] = match2;
          currentMessage = {
            sender: sender.trim(),
            timestamp: this.parseTimestamp(timestampStr),
            text: textContent.trim(),
