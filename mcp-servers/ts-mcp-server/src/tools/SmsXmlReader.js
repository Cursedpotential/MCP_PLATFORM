    ...
    parseElementToDocument(xml) {
        try {
            ...
            if (isCall) {
                ...
                const blockEvidence = [];
                if (type === '5')
                    blockEvidence.push("Call actively rejected");
                if (type === '6')
                    blockEvidence.push("Number on refuse/block list");
                if (type === '2' && duration === '0')
                    blockEvidence.push("Outgoing call with 0 duration - did not connect");
...
exports.SmsXmlReader = SmsXmlReader;
//# sourceMappingURL=SmsXmlReader.js.map